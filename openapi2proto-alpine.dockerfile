...
# build: docker build -t openapi2proto:alpine -f openapi2proto-alpine.dockerfile --no-cache .
# run: docker run --rm -ti -p 8084:8084 -v `pwd`:/app openapi2proto:alpine
...
# env
ENV APP_BASENAME=${APP_BASENAME:-"openapi2proto"} \
...
COPY ./src/github.com/NYTimes/openapi2proto /go/src/github.com/NYTimes/openapi2proto
WORKDIR /go/src/github.com/NYTimes/openapi2proto

RUN glide install --strip-vendor \
	&& gox -verbose -os="linux" -arch="amd64" -output="/app/${APP_BASENAME}-{{.Dir}}" $(glide novendor)
...
# CMD ["/usr/local/sbin/gosu", "app", "${APP_BASENAME}"]
